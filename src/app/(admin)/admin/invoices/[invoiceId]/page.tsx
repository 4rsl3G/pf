...
export default function AdminInvoiceDetail() {
  ...
  async function refetchReceipt() {
    ...
    try {
      setBusy(true);
      await apiFetch<any>(`/admin/invoices/${invoiceKey}/refetch-receipt`, {
        method: "POST",
        auth: true,
      });
        ...
        <div className="grid gap-4 lg:grid-cols-[1.35fr_.65fr] min-w-0">
          ...
          <Card className="card-glass border-soft rounded-2xl overflow-hidden min-w-0">
            ...
            <CardContent className="space-y-4 min-w-0">
              {/* ACTION BAR — FIX MOBILE OVERFLOW */}
  ...
  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between min-w-0">
      <div className="min-w-0">
        ...
        {/* mobile: wrap, desktop: truncate */}
        <div className="text-xl sm:text-2xl font-semibold break-words sm:truncate">
          {title}
        </div>

        {subtitle ? (
          <div className="text-sm text-subtle mt-1 break-words sm:truncate">{subtitle}</div>
        ) : null}
      </div>
